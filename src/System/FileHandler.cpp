...
// note: we only need FileExists(), Seek(), Read(), GetPos()
...
bool CFileHandler::TryRawFS(const string& filename) {
	ifs = new std::ifstream(filename.c_str(), ios::in | ios::binary);
...
bool CFileHandler::TryModFS(const string& filename) {
	...
	if (hpiLength != -1) {
		...
		// CVFSHandler::LoadFile() can be implemented by Archive{Buffered, Dir, HPI} ...
...
vector<string> CFileHandler::DirList(const string& path, const string& pattern, const string& modes) {
...
bool CFileHandler::InsertRawFiles(set<string>& fileSet, const string& path, const string& pattern) {
	boost::regex regexpattern(filesystem.glob_to_regex(pattern), boost::regex::icase);
...
bool CFileHandler::InsertModFiles(set<string>& fileSet, const string& path, const string& pattern) {
	if (!vfsHandler) {
		return false;
	}
...
bool CFileHandler::InsertMapFiles(set<string>& fileSet, const string& path, const string& pattern) {
	return InsertModFiles(fileSet, path, pattern); // FIXME
}

bool CFileHandler::InsertBaseFiles(set<string>& fileSet, const string& path, const string& pattern) {
	return InsertModFiles(fileSet, path, pattern); // FIXME
}



vector<string> CFileHandler::SubDirs(const string& path, const string& pattern, const string& modes) {
	const string pat = pattern.empty() ? "*" : pattern;
...
bool CFileHandler::InsertRawDirs(set<string>& dirSet, const string& path, const string& pattern) {
	boost::regex regexpattern(filesystem.glob_to_regex(pattern), boost::regex::icase);
	vector<string> found = filesystem.FindFiles(path, pattern, FileSystem::ONLY_DIRS);
	vector<string>::const_iterator fi;
...
bool CFileHandler::InsertModDirs(set<string>& dirSet, const string& path, const string& pattern) {
	if (!vfsHandler) {
		return false;
	}
...
bool CFileHandler::InsertMapDirs(set<string>& dirSet, const string& path, const string& pattern) {
	return InsertModDirs(dirSet, path, pattern); // FIXME
}

bool CFileHandler::InsertBaseDirs(set<string>& dirSet, const string& path, const string& pattern) {
	return InsertModDirs(dirSet, path, pattern); // FIXME
}
